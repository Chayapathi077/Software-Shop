...
import { Contract, JsonRpcProvider, Wallet, ethers, Interface } from 'ethers';
...
export async function createUser(userData: {username: string, email: string, password: string, securityPhrase: string, panNumber: string, walletAddress: string}): Promise<{ success: boolean; message: string }> {
  try {
    ...
    // Ensure wallet address is valid before doing anything else
    if (!ethers.isAddress(walletAddress)) {
      return { success: false, message: 'The provided wallet address is invalid.' };
    }
    ...
    const newUser: any = {
      ...
      securityPhraseHash,
      panNumberHash,
      walletAddress: ethers.getAddress(walletAddress), // Ensure checksummed address is stored
      createdAt: new Date(),
    };
...
/**
 * Deletes a user account and all associated data, including software and licenses.
...
export async function deleteUserAccount(username: string): Promise<{ success: boolean; message: string }> {
  ...
  try {
    let resultMessage = "Account and all associated data deleted successfully.";
    await session.withTransaction(async () => {
      // 1. Find the user to get their ID
      const user = await db.collection('users').findOne({ username }, { session });
      if (!user) {
        // We can't throw an error inside a transaction easily, so we'll set a message and return.
        resultMessage = "User not found.";
        return; 
      }

      // 2. Find all software created by this user
      const softwareToDelete = await db.collection('software').find({ sellerId: user._id }, { projection: { _id: 1 }, session }).toArray();
      const softwareIds = softwareToDelete.map(s => s._id);

      if (softwareIds.length > 0) {
        // 3. Delete all licenses associated with that software
        await db.collection('licenses').deleteMany({ softwareId: { $in: softwareIds } }, { session });
        
        // 4. Delete all software uploaded by the user
        await db.collection('software').deleteMany({ sellerId: user._id }, { session });
      }

      // 5. Delete the user account itself
      const result = await db.collection('users').deleteOne({ _id: user._id }, { session });

      if (result.deletedCount === 0) {
        resultMessage = "Failed to delete the user account.";
      }
    });
...
export async function recordLicensePurchase(licenseData: LicenseData): Promise<{ success: boolean; message: string }> {
  ...
  if (!softwareId || !buyerAddress || tokenId === undefined || !transactionHash) {
      return { success: false, message: "Missing required license data for recording." };
  }
  ...
  try {
    ...
    if (!software) {
      return { success: false, message: "The purchased software does not exist in the database." };
    }
...
/**
 * Fetches all licenses owned by a specific buyer that have not been deleted by them.
...
export async function getLicensesForBuyer(buyerAddress: string): Promise<any[]> {
    ...
    try {
        ...
        const licenses = await db.collection('licenses').find({ 
            buyerAddress: buyerAddress.toLowerCase(),
            status: { $ne: 'deleted_by_user' }
        }).sort({ mintDate: -1 }).toArray();
 ...
 * It also now implements auto-blocking on device violation.
 * @param licenseId The ID of the license.
 * @param deviceId The unique identifier of the device.
...
export async function getDecryptionKey(licenseId: string, deviceId: string, walletAddress: string): Promise<{ success: boolean; message: string; key?: string; fileUrl?: string; }> {
  ...
  if (!AMOY_RPC_URL) {
      console.error("Server configuration error: AMOY_RPC_URL is not set.");
  ...
  try {
    ...
    // Ensure the wallet using the license file is the original buyer.
    if (license.buyerAddress.toLowerCase() !== walletAddress.toLowerCase()) {
        return { success: false, message: "Wallet address does not match the license owner. This license file cannot be used with this wallet." };
    }
    
    // Primary check: license status in our DB. If not active, fail fast.
    if (license.status !== 'active') {
        let userMessage = `This license is currently ${license.status.replace(/_/g, ' ')}.`;
        if (license.status === 'blocked') userMessage += " Contact the seller for assistance.";
        if (license.status === 'deleted_by_user') userMessage = "This license has been removed from your account and cannot be used.";
    ...
    // DEVICE LOCK VALIDATION & AUTO-BLOCKING
    if (software.licensingRules.fingerprintLock) {
        if (license.deviceId && license.deviceId !== deviceId) {
            // VIOLATION DETECTED!
            // 1. Block the license in the database
            await db.collection('licenses').updateOne(
              { _id: new ObjectId(licenseId) },
              { $set: { status: 'blocked', reason: 'Device violation', lastViolationDate: new Date() } }
            );
            // 2. Send an email notification to the seller (fire-and-forget)
            sendViolationEmail(license).catch(console.error);
            // 3. Return error to the user
            return { success: false, message: "Device mismatch. This license has been automatically blocked for security reasons. Please contact the seller." };
        }
    }
...
export async function revokeLicense(licenseId: string): Promise<{ success: boolean; message: string }> {
    if (!SELLER_PRIVATE_KEY || !AMOY_RPC_URL) {
        return { success: false, message: "Server is not configured for blockchain transactions. Missing seller private key or RPC URL." };
    }
    ...
    try {
        ...
        const provider = new JsonRpcProvider(AMOY_RPC_URL);
        const signer = new Wallet(SELLER_PRIVATE_KEY, provider);
...
export async function reactivateLicense(licenseId: string): Promise<{ success: boolean; message:string }> {
    ...
    try {
        ...
        if (license.status === 'deleted_by_user') {
            return { success: false, message: "Cannot reactivate a license that the user has removed from their account."}
        }
...
export async function deleteSoftware(softwareId: string): Promise<{ success: boolean; message: string }> {
    ...
    try {
        let resultMessage = "Software and all associated licenses have been deleted.";
        await session.withTransaction(async () => {
            const sId = new ObjectId(softwareId);

            const softwareExists = await db.collection('software').findOne({ _id: sId }, { session });
            if (!softwareExists) {
                resultMessage = "Software not found.";
                return; // Abort transaction
            }

            // Delete all licenses associated with this software
            await db.collection('licenses').deleteMany({ softwareId: sId }, { session });

            // Delete the software itself
            const result = await db.collection('software').deleteOne({ _id: sId }, { session });

            if (result.deletedCount === 0) {
                 resultMessage = "Failed to delete the software.";
            }
        });
        
        if (resultMessage.includes("deleted")) {
            return { success: true, message: resultMessage };
        } else {
            return { success: false, message: resultMessage };
        }
