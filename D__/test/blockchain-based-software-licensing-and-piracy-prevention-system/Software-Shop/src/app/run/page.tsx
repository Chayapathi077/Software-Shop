...
// This function decrypts the raw ciphertext buffer and returns a WordArray
function decryptData(encryptedBuffer: ArrayBuffer, key: string): CryptoJS.lib.WordArray {
    try {
        const keyHex = CryptoJS.enc.Hex.parse(key);
        const iv = CryptoJS.enc.Hex.parse('00000000000000000000000000000000'); 
        
        // Convert ArrayBuffer to WordArray
        const ciphertext = CryptoJS.lib.WordArray.create(encryptedBuffer);

        const decrypted = CryptoJS.AES.decrypt({ ciphertext: ciphertext } as any, keyHex, {
            iv: iv,
        ...
        if (!decrypted || decrypted.sigBytes <= 0) {
           throw new Error("Decryption resulted in empty content. The key may be incorrect or the ciphertext corrupted.");
        }
...
export default function RunSoftwarePage() {
    ...
    // Clean up blob URL when component unmounts or when a new file is loaded
    useEffect(() => {
        return () => {
            if (iframeSrc) {
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            if (file.name.endsWith('.license.json')) {
                setLicenseFile(file);
                // Automatically trigger load on file select for a smoother UX
    ...
    const handleLoadSoftware = async (fileToLoad: File) => {
        ...
        try {
            ...
            const originalFileName = fullFileName.replace('.enc', '');
            setFileName(originalFileName);

            toast({ title: "Decrypting...", description: "Unlocking file content locally in your browser." });
            const decryptedWordArray = decryptData(encryptedArrayBuffer, decryptionKey);

            // Convert WordArray to a Uint8Array, then create a Blob
            const latin1 = CryptoJS.enc.Latin1.stringify(decryptedWordArray);
            const uint8Array = new Uint8Array(latin1.length);
            for(let i=0; i<latin1.length; i++){
                uint8Array[i] = latin1.charCodeAt(i);
            }
            
            const mimeType = originalFileName.endsWith('.html') ? 'text/html' : 'application/octet-stream';
            const blob = new Blob([uint8Array], { type: mimeType });

            if (originalFileName.endsWith('.html')) {
                // For HTML, create a blob URL and render in iframe
                const localUrl = URL.createObjectURL(blob);
                setIframeSrc(localUrl);
                toast({ title: "Success!", description: "Software loaded and is running in the sandbox below." });
            } else {
                // For all other file types, trigger a download
                ...
                toast({ title: "Success!", description: `Your file "${originalFileName}" has been downloaded.`, duration: 9000 });
            }
            ...
            setIsLoading(false);
            // Optionally clear the file input after processing
            // setLicenseFile(null);
        ...
        <div className="flex min-h-screen w-full flex-col bg-gradient-to-br from-primary to-accent text-white">
            <header className="sticky top-0 flex h-16 items-center justify-between px-4 md:px-6 z-40">
                <Link href="/dashboard" className="flex items-center gap-2 text-lg font-semibold md:text-base">
                    <div className={cn("relative flex items-center justify-center h-10 w-10")}>
                        <div className={cn("relative flex items-center justify-center bg-white/10 backdrop-blur-md border border-white/20 shadow-lg transition-all duration-1000 ease-in-out rounded-lg", "h-10 w-10 p-2")}>
                            <div className="flex items-center justify-center w-full h-full">
                                <Zap className={cn("text-white transition-all duration-1000 ease-in-out absolute", "h-5 w-5 -translate-x-1")} />
                                <Zap className={cn("text-white transition-all duration-1000 ease-in-out absolute", "h-5 w-5 translate-x-1")} />
                            </div>
                        </div>
                    </div>
            ...
            <main className="flex flex-1 flex-col items-center gap-4 p-4 md:gap-8 md:p-8">
                <div className="w-full max-w-2xl rounded-xl border border-white/20 bg-white/10 p-8 shadow-lg backdrop-blur-xl space-y-6">
                    <div className="text-center">
                        <h1 className="text-2xl font-bold">Access &amp; Download Software</h1>
                        <p className="text-gray-300">Upload your `.license.json` file to securely access your software.</p>
                    </div>
                    ...
                    <div className="space-y-4">
                        <label htmlFor="license-upload" className={cn("relative flex w-full h-32 border-2 border-white/30 border-dashed rounded-lg cursor-pointer bg-white/5 hover:bg-white/10 transition-colors", { "items-center justify-center": !licenseFile })}>
                           { !licenseFile ? (
                               ...
                               <div className="relative flex items-center justify-between w-full p-3 h-full">
                                    ...
                                    <Button variant="ghost" size="icon" className="text-white hover:bg-white/20 h-7 w-7 flex-shrink-0 z-10" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setLicenseFile(null); if (iframeSrc) { URL.revokeObjectURL(iframeSrc); setIframeSrc(null); } const input = document.getElementById('license-upload') as HTMLInputElement; if(input) input.value = ''; }}>
                                        <X className="h-4 w-4"/>
                                    </Button>
                                </div>
                           )}
                           <input id="license-upload" type="file" accept=".json,.license.json" className="sr-only" onChange={handleFileChange} disabled={isLoading} />
                        </label>
                        ...
                        >
                            ...
                            {isLoading ? 'Validating and Loading...' : 'Access Software'}
                        </Button>
                    </div>
                </div>
                 {iframeSrc && (
                    <div className="w-full max-w-4xl rounded-xl border border-white/20 bg-black/20 p-4 shadow-lg backdrop-blur-xl mt-8 space-y-4">
                         <h2 className="text-xl font-bold text-center">Running Software: {fileName}</h2>
                         <iframe
                            src={iframeSrc}
                            sandbox="allow-scripts allow-same-origin"
                            className="w-full h-[60vh] bg-white rounded-md border-2 border-white/20"
