...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
        if (window.ethereum) {
            try {
                // Use eth_accounts to check for existing permissions without popping up MetaMask
                ...
                if (accounts.length > 0) {
                    ...
                    // No accounts connected in MetaMask, clear session storage if it exists
  ...
  const handleConnectWallet = async () => {
    ...
    try {
      ...
      // Always request accounts to force connection prompt if needed
