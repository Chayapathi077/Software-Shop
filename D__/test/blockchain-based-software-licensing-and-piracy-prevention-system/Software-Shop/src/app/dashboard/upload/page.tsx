...
import { ArrowLeft, UploadCloud, File, X, Wallet, Fingerprint, KeyRound, Zap } from 'lucide-react';
                            ...
                            {!file ? (
                                <label htmlFor="file-upload" className="relative flex flex-col items-center justify-center w-full border-2 border-white/30 border-dashed rounded-lg cursor-pointer bg-white/5 hover:bg-white/10 transition-colors flex-grow min-h-[160px]">
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p className="text-xs text-gray-400">e.g., .zip, .exe, .html</p>
                                    </div>
                                    <Input id="file-upload" type="file" className="sr-only" onChange={handleFileChange} disabled={isUploading} accept=".zip,.exe,.msi,.dmg,.pkg,.deb,.html,.txt,.pdf,.jpg,.png"/>
                                </label>
                            ) : (
