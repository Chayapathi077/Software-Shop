...
export default function SignUpDetailsPage() {
    ...
    const handleConnectMetaMask = async () => {
        ...
        try {
            ...
            if (accounts.length > 0) {
                // IMPORTANT: Validate and checksum the address immediately.
                const checksummedAddress = ethers.getAddress(accounts[0]);
                setWalletAddress(checksummedAddress);
                toast({
                    title: "Wallet Connected",
                    description: `Successfully connected to address: ${checksummedAddress.substring(0, 6)}...${checksummedAddress.substring(checksummedAddress.length - 4)}`,
                });
            }
        } catch (error: any) {
    ...
    const handleContinue = () => {
        ...
        // Ensure wallet address is a valid checksummed address before proceeding
        if (!panNumber || !walletAddress || !ethers.isAddress(walletAddress)) {
            toast({ title: "Error", description: "A valid PAN number and wallet address are required.", variant: "destructive" });
            return;
        }
