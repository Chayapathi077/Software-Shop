...
import { ArrowLeft, Download, ShieldCheck, ExternalLink, Fingerprint, Ban, Rocket, Zap } from 'lucide-react';
...
export default function BuyerDashboardPage() {
  ...
  useEffect(() => {
    const checkConnection = async () => {
        ...
        if (storedAddress) {
            ...
            await fetchLicenses(storedAddress);
            return;
        }
  ...
  const handleDownloadLicenseFile = async (license: License) => {
    ...
    try {
        // First, attempt to bind the device. This is a safe pre-check.
        const bindResult = await bindDeviceToLicense(license._id, getDeviceId());
        if (!bindResult.success) {
            // If binding fails (e.g., license already bound to another device), show the error.
            throw new Error(bindResult.message);
        }

        // Fetch just the encrypted file URL without full key validation.
        ...
        <div className="flex items-center gap-2">
            <TooltipProvider>
                <Tooltip>
                    ...
                    <TooltipContent>
                        <p>Go to the Run page to load your downloaded license file.</p>
                    </TooltipContent>
                </Tooltip>
            </TooltipProvider>
                             ...
                             <TableRow key={license._id} className="border-b-white/20 hover:bg-white/10 last:border-b-0">
                                ...
                                <TableCell className="text-center">
                                     <div className='flex items-center justify-center gap-2 text-sm'>
                                        ...
                                        {license.status === 'blocked' && <Ban className="h-5 w-5 text-yellow-500"/>}
                                        {license.status === 'revoked' && <Ban className="h-5 w-5 text-red-500"/>}
                                        ...
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <Button 
                                                    ...
                                                    disabled={license.status !== 'active' || isDownloading === license._id}
                                                >
